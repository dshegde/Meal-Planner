...
export async function planner_routes(app: FastifyInstance): Promise<void> {
	...
	app.delete("/mealplan/:userid", async (req: any, reply) => {
		const userid = req.params.userid;
		console.log(userid);
		const query = app.db.mp.createQueryBuilder("mp").where("userId = :id", { id: userid }).delete().execute();
	...
	app.delete("/mealplan/:userid/:dayOfWeek/:mealType", async (req: any, reply) => {
	...
	app.put<{
		...
		let dayOfWeekOptions = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"];
		...
		if (!userId || !mealType || !dayOfWeek || !recipeId) {
			...
			reply.status(400).send(errMsg);
		} else if (!mealTypeOptions.includes(mealType) || !dayOfWeekOptions.includes(dayOfWeek)) {
			let errMsg = {
	...
	app.post<{
		...
		if (!recipeName || !dietType || !cuisine || !description || !ingredients) {
			let errMsg = {
				error: "recipeName,description,cuisine,dietType and ingredients are required",
			};
			reply.status(400).send(errMsg);
		} else {
	...
	app.post<{
		...
		var dayOfWeekOptions = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"];
		const { userId, mealType, dayOfWeek, recipeId } = req.body;
		if (!userId || !mealType || !dayOfWeek || !recipeId) {
			// there has to be a better check to see if mealType and dayOfWeek are valid
			let errMsg = {
