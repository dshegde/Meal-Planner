...
export async function planner_routes(app: FastifyInstance): Promise<void> {
	...
	// 	let errMsg = {
	// 		error: `No record found for ${paramName}: ${paramValue}`,
	// 	};
  ...
  app.get("/recipes/cuisine/:cuisine", async (req: any, reply) => {
  	...
  	if (recipe.length == 0) {
  		handleNotFoundResponse(reply, [{ name: 'cuisine type', value: req.params.cuisine }]);
  	} else {
  		reply.send(recipe);
  	}
  });
  ...
  app.get("/recipes/dietType/:dietType", async (req: any, reply) => {
  	...
  	if (dietType.length == 0) {
  		handleNotFoundResponse(reply, [{ name: 'Diet type', value: req.params.dietType }]);
  	} else {
  		reply.send(dietType);
  	}
  });
  ...
  app.get("/recipes/:cuisine/:dietType", async (req: any, reply) => {
  	...
  	if (recipes.length == 0) {
  		handleNotFoundResponse(reply, [
  			{ name: 'cuisine type', value: req.params.cuisine },
  			{ name: 'diet type', value: req.params.dietType },
  		]);
  	} else {
  		reply.send(recipes);
  	}
